 ...
 * 1. Neutralize the revenue (debit revenue, credit 1221 unbilled pledge receivable)
 * 2. Net effect: Convert 1221 unbilled pledge receivable to 1220 regular AR
 */
...
function processPledgeInvoiceAdjustments(transactionRecord, standardLines, customLines, sourceSORecord) {
    ...
    // Step 1: Neutralize each revenue line (debit revenue, credit 1221)
    var totalRevenueNeutralized = 0;
    var customSegs = ['cseg_npo_region', 'cseg_project', 'cseg_npo_restrictn'];
    for (var j = 0; j < revenueLines.length; j++) {
        ...
        // Copy all standard segments
        try {
            var deptId = revenueLine.line.getDepartmentId();
            if (deptId) revenueDebitLine.setDepartmentId(deptId);

            var classId = revenueLine.line.getClassId();
            if (classId) revenueDebitLine.setClassId(classId);

            var locId = revenueLine.line.getLocationId();
            if (locId) revenueDebitLine.setLocationId(locId);

            // Copy all custom segments by documented method
            for (var s = 0; s < customSegs.length; s++) {
                var segVal = null;
                try {
                    segVal = revenueLine.line.getSegmentValueId(customSegs[s]);
                } catch (err) {
                    nlapiLogExecution('ERROR', 'getSegmentValueId error on segment: ' + customSegs[s], err.toString());
                }
                if (segVal) {
                    try {
                        revenueDebitLine.setSegmentValueId(customSegs[s], segVal);
                        nlapiLogExecution('DEBUG', 'Copied ' + customSegs[s], 'Value: ' + segVal);
                    } catch (err2) {
                        nlapiLogExecution('ERROR', 'setSegmentValueId error on segment: ' + customSegs[s], err2.toString());
                    }
                } else {
                    nlapiLogExecution('DEBUG', 'No value for segment ' + customSegs[s]);
                }
            }
        } catch (segmentError) {
            nlapiLogExecution('DEBUG', 'Error copying GL segments', segmentError.toString());
        }
    ...
    // Step 2: Create credit to 1221 to balance the revenue debits
    if (totalRevenueNeutralized > 0) {
        var pledgeReceivableCreditLine = customLines.addNewLine();
        pledgeReceivableCreditLine.setAccountId(741); // 1221 Unbilled Pledge Receivables
        ...
        if (entityId) {
            try {
                ...
                nlapiLogExecution('DEBUG', 'Could not set entity on 1221 credit line', e.toString());
            }
        }
        pledgeReceivableCreditLine.setMemo('Reducing Unbilled Pledge Receivable upon Invoicing - No Net AR Impact');
        
        nlapiLogExecution('AUDIT', 'Created 1221 Credit Line', 
    ...
    if (totalRevenueNeutralized > 0) {
        var pledgeReceivableCreditLine = customLines.addNewLine();
        pledgeReceivableCreditLine.setAccountId(741); // 1221 Pledge Order Receivables
        pledgeReceivableCreditLine.setCreditAmount(totalRevenueNeutralized);
    ...
    */
    ...
    nlapiLogExecution('AUDIT', 'Pledge Invoice GL Adjustments Complete', 
        ...
        ', Net Effect: Converted 1221 pledge receivable to 1220 regular AR');
    
    // Log the accounting effect
    nlapiLogExecution('AUDIT', 'ACCOUNTING EFFECT SUMMARY', 
        'BEFORE: 1221 Pledge Receivable (higher), Revenue already recognized' +
        ' AFTER: 1220 Regular AR +$' + arLine.debitAmount + ', Revenue neutralized, 1221 reduced by $' + totalRevenueNeutralized +
        ' RESULT: Unbilled Pledge Receivable converted to AR Aging, no duplicate revenue');
}
